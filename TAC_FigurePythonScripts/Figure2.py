...
for its in range(2,max_it+1):
	...
	for aves in range(num_ave):
		...
		while agents[0].matching_exists:
			...
			sb.aug_path(adj,agents,D)
			DFS_counter[its-2] = DFS_counter[its-2] + 1

		while agents2[0].matching_exists:
			sb.arg_max(adj,agents2,D)
			sb.aug_path(adj,agents2,D)
			BFS_counter[its-2] = BFS_counter[its-2] + 1
...
plt.ylabel("Average iterations of pruneBAP",fontsize=18)
...
np.save('itvsn_DFS_sparse.npy', np.array(DFS_counter))
np.save('itvsn_BFS_sparse.npy', np.array(BFS_counter))
